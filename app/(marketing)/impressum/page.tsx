        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto space-y-8">
                ...
                {companyInfo.map((info, index) => {
                  ...
                  return (
                    <div key={info.label} className="flex items-start gap-4">
                      <Icon className="h-5 w-5 text-text-muted mt-1 flex-shrink-0" />
                      <div>
                        <div className="text-sm font-inter text-text-muted mb-1">{info.label}</div>
                        <div className="font-inter text-text-primary">{info.value}</div>
                      </div>
                    </div>
                  )
                })}
            ...
            >
              ...
              <div className="space-y-4">
                {legalInfo.map((info) => (
                  <div key={info.label} className="flex justify-between items-start border-b border-border-subtle pb-3">
                    <div className="text-sm font-inter text-text-muted">{info.label}</div>
                    <div className="font-inter text-text-primary">{info.value}</div>
                  </div>
                ))}
              </div>
