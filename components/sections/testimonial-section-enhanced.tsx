    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-black/70 font-inter">
          ...
          {testimonials.map((testimonial, index) => (
            ...
            >
              ...
              <div className="absolute top-6 right-6 opacity-10 group-hover:opacity-20 transition-opacity">
                <Quote className="h-12 w-12 text-black" strokeWidth={1.5} />
              </div>

              {/* Quote */}
              <blockquote className="text-lg font-inter text-black/80 mb-6 pr-8">
                &ldquo;{testimonial.quote}&rdquo;
              </blockquote>

              {/* Author Info */}
              <div className="flex items-start justify-between gap-4 pt-6 border-t border-black/10">
                <div className="flex-1">
                  <p className="font-jakarta font-medium text-black mb-1">
                    {testimonial.author.name}
                  </p>
                  <p className="text-sm text-black/50 font-inter mb-4">
                    {testimonial.author.role} · {testimonial.author.company}
                  </p>

                  {/* Favorite Features */}
                  <div>
                    <p className="text-xs text-black/50 font-inter mb-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {testimonial.favoriteFeatures.map((feature) => (
                        <Badge
                          key={feature}
                          variant="outline"
                          className="text-xs border-black/10 text-black/70 bg-white/50"
                        >
                          {feature}
                        </Badge>
                      ))}
