...
export function CalendarEvent({ title, date, time, type = "meeting", className }: CalendarEventProps) {
  return (
    <motion.div
      className={`flex items-center gap-3 p-3 bg-surface rounded-lg hover:bg-surface-elevated transition-all cursor-pointer group ${className || ""}`}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-inter text-text-primary font-medium mb-0.5">
          {title}
        </p>
