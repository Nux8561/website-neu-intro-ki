...
const ScannerCardStream = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
      ...
      if (cardLineRef.current) {
        cardLineRef.current.style.transform = `translateX(${cardStreamState.current.position}px)`;
      }
    };
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      cardStreamState.current.velocity = Math.max(30, Math.min(300, cardStreamState.current.velocity + delta * 0.1));
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      {showSpeed && (
        <div className="absolute top-4 right-4 z-30 px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white text-sm">
          Speed: {speed}
        </div>
      )}
          ...
          {cards.map(card => (
            <div key={card.id} className="card-wrapper relative w-[400px] h-[250px] shrink-0">
              ...
              <div className="card-ascii card absolute top-0 left-0 w-full h-full rounded-[15px] overflow-hidden bg-transparent z-[1] [clip-path:inset(0_calc(100%-var(--clip-left,0%))_0_0)]">
                <pre className="ascii-content absolute top-0 left-0 w-full h-full text-[rgba(75,85,99,0.8)] font-mono text-[11px] leading-[13px] overflow-hidden whitespace-pre m-0 p-0 text-left align-top box-border [mask-image:linear-gradient(to_right,rgba(0,0,0,1)_0%,rgba(0,0,0,0.8)_30%,rgba(0,0,0,0.6)_50%,rgba(0,0,0,0.4)_80%,rgba(0,0,0,0.2)_100%)] animate-glitch">
                  {card.ascii}
                </pre>
              </div>
            </div>
